...
function spaceAdvances(key, callback) {
  console.log("LOG: defining space bahavior with code: " + key);
	// Define a wrapper that checks for the 'space' key and executes the callback
	const wrappedCallback = (event) => {
    console.log(`now: ${Date.now()} last space: ${CURRENT.spaceHandler.last_space} sub: ${Date.now() - CURRENT.spaceHandler.last_space}`)
		if (
